    ...
    private void setListeners() {
        ...
        ItemTouchHelper.SimpleCallback simpleItemTouchCallback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(RecyclerView.ViewHolder viewHolder, int swipeDir) {
                ShoppingList deleted = mViewModel.getLists().getValue().get(viewHolder.getAdapterPosition());
                if (deleted.id == mViewModel.getSelectedListID().getValue()) {
                    ShoppingList firstList = mViewModel.getLists().getValue().get(0);
                    if (firstList != null) {
                        mViewModel.setSelectedListID(firstList.id);
                    }
                }
                mListRepository.deleteList(deleted.id);
            }
        };
